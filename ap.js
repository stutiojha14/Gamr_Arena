...
$scope.totoalResult = results.length-1;
	$scope.categories = [];
	var fakeCat =[];

	results.forEach((item) => {
		if(item.platform && fakeCat.indexOf(item.platform) ==-1){
			var obj = {};
			obj.plateform = item.platform;
			fakeCat.push(item.platform);
	...
	$scope.Search = function(title){		
		if(title===""){
			...
			$scope.games = 	searchedItem;
			$scope.totoalResult = $scope.games.length;
		}
	}

	$scope.sort = function(type){
		if(type==='asc'){
			$scope.games.sort((a,b)=>{
	...
	$scope.detect = (i)=>{
		...
		$scope.games = games;
		$scope.totoalResult = games.length;
	};
}]);
